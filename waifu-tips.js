...
live2d_settings['modelTexturesId']      = 46;           // 默认材质 ID，可在 F12 控制台找到
...
live2d_settings['waifuMinWidth']        = '1023px';      // 面页小于 指定宽度 隐藏看板娘，例如 'disable'(禁用), '768px'
live2d_settings['waifuEdgeSide']        = 'left:10px';     // 看板娘贴边方向，例如 'left:0'(靠左 0px), 'right:30'(靠右 30px)
...
function showMessage(text, timeout, flag) {
    if(flag || sessionStorage.getItem('waifu-text') === '' || sessionStorage.getItem('waifu-text') === null){
        ...
        timeout = text.length / 5 * 1000 + 3500;
        hideMessage(timeout);
    }
}
        ...
        $.ajax({
            ...
            success: function(result) {
                if (result.textures['id'] == 1 && (modelTexturesId == 1 || modelTexturesId == 0))
                    showMessage(waifu_tips.load_rand_textures[0], 5000, true);
                else showMessage(waifu_tips.load_rand_textures[1], 5000, true);
                ...
                $.ajax({
                    ...
                    success: function (result, status) {
                        if (!empty(result.data.origin.title)) {
                            ...
                            window.setTimeout(function() {showMessage(text, 3000, true);}, result.data.content.length/5*1000+3500);
    	    ...
    	    default:
    	        $.getJSON('https://v1.hitokoto.cn',function(result){
            	    if (!empty(result.from)) {
                        ...
                        var tmot = result.hitokoto.length/5*1000+3500;
                        window.setTimeout(function() {showMessage(text, 3000, true);},tmot);
                        console.log("时间："+tmot);
            	    }
                    showMessage(result.hitokoto, 5000, true);
                });
